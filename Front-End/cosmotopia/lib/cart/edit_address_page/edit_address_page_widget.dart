                        ...
                        child: FFButtonWidget(
                          onPressed: () async {
                            if (_model.textController1.text == _model.textController2.text) {
                              final address = _model.textController1.text;
                              FFAppState().address = address;
                              final response = await ApiService.updateAddress(address, token: FFAppState().token);
                              if (response.statusCode == 200) {
                                Navigator.pop(context);
                              } else {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(content: Text('Update address failed!')),
                                );
                              }
                            } else {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(content: Text('Addresses do not match')),
                              );
                            }
                          },
                          text: 'Add',
